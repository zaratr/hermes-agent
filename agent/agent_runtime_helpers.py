...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if agent.provider == "copilot-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://copilot"):
        ...
        return client
    if agent.provider == "antigravity-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://antigravity"):
        from agent.antigravity_acp_client import AntigravityACPClient

        client = AntigravityACPClient(**client_kwargs)
        _ra().logger.info(
            "Antigravity ACP client created (%s, shared=%s) %s",
            reason,
            shared,
            agent._client_log_context(),
        )
